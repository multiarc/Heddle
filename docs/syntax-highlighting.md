...
- **`@@` is not an escape.** The TextMate grammar still ships a rule that colors `@@` as a literal‑`@`
  escape, but no such escape exists — `@@` is a compile error (use `@{ … }@` or `@:` for a literal
  `@`). The highlight is cosmetic only; the token does not compile.
