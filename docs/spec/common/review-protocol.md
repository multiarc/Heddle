...
[`findings-register.md`](findings-register.md) is short and is kept short. It
...
2. **Real, unfixed, deterministically reproducible in-process** → write the
   ...
   is the fixer's acceptance evidence. The skip-string form above is the
   normative one; "quarantined, never weakened — an unexplained or orphaned
   skip is a review failure" is in
   [`testing-standards.md`](testing-standards.md) (precompiled-tier posture).
...
[`coding-standards.md`](coding-standards.md) § "SOLID, DRY, and YAGNI in
balanced mode" (precedence: correctness → sandbox security → simplicity →
measured performance → abstraction last; a seam is added when a second
consumer exists, not because one might appear). Precedent for recorded
limitations: the findings register's unverified-platform-surface section.
