...
> **Note (2026-09-18).** Every "ships in 2.1.0" / "lands in 2.1" below means the 3.0.0 release: 2.1
> was retired without a release and its items ship under the [3.0 window](#30-window--pending-tag-21-retired-without-a-release).
...
## 3.0 window — pending tag (2.1 retired without a release)

**2.1 was ratified as a breaking window** (maintainer ruling, 2026-07-26; binary changes and minor API
changes or additions) **and was never released.** No `v2.1.0` tag exists. On 2026-09-18 the maintainer
retired the window without a release: every item implemented for it ships in **3.0.0**, under the v3
window (its planning record, `v3-window-closed.md`, is retired with the plan: `git show f8a9497c:docs/plan/precompilation-v2/`). Wherever a record in
this repository says "ships in 2.1.0", read 3.0.0.

The per-item judgements [above](#explicit-not-window-gated-rulings) remain accurate about *why each
change is safe*. Policy rule 5 still governs: the record below is **as implemented**, and it is
reconciled against the shipped source when `v3.0.0` is tagged, then condensed into
[cross-cutting-decisions.md § Release records](cross-cutting-decisions.md#release-records--as-shipped).
There is deliberately no "as shipped" record before the tag.

### The 2.1 items, condensed

The full 2.1 table (2.0.0 state and as-implemented text per item) is in git history:
`git show f8a9497c:docs/spec/common/breaking-windows.md`.
...
| 5 | Strong naming of `Heddle.Demo.Models` / `Heddle.Demo.Wasm` (Q8.11) | Ships as implemented | `heddle.snk`; the declared Scriban exception in `Directory.Build.targets` |
| 6 | Registered `Name` as a runtime lookup spelling (Q8.30) | Ships as implemented | `PrecompiledTemplateInfo.RegisteredName`, the registry's second index, `HED7104` |
| 7 | Opted-out items validated and advised (Q8.28 / Q8.29) | Ships as implemented | A `Precompile="false"` item draws `HED7004` / `HED7028` like any other (CHANGELOG 3.0.0) |
| 8 | `PrecompiledFallbackEvent` carriers (Q8.33) | Ships as implemented | `Key` removed; `TemplateKey` / `AssemblyName` through `ForTemplate` / `ForAssembly` (CHANGELOG 3.0.0 § Changed (breaking)) |
| 9 | Unnormalizable manifest `Name` (Q8.32) | Ships as implemented | `HED7104` / `RegisteredNameUnavailable` at registration |
| 10 | Assembly auto-loading and scan-all extension discovery removed (Q8.37) | Ships as implemented | The engine loads and scans nothing on its own ([D11](cross-cutting-decisions.md#d11--the-engine-does-not-decide-which-assemblies-are-loaded); CHANGELOG 3.0.0) |
| 11 | Precompilation-coverage widening — the 2.x generator program | Superseded by the compiled form | Coverage is the engine's own compile, serialized; what does not precompile is a refusal class declared per corpus row (`CorpusIntent`) and reported as `HED7031` |
| 12 | Binding the extensions instead of predicting them | Superseded | `InitStart` / `CompleteInit` run for real at materialization, over bodies served from the form |
| 13 | Binding the child-template host (`@partial`, `[ChildTemplateHost]`) | Superseded; the register's `ResolvePartial` removal row is adopted here | Named children resolve from the artifact through the form cursor; `PrecompiledRuntime` and its partial API are deleted |
| 14 | Binding the slot projection (`@out`, `[SlotProjection]`) | Superseded | Slot bodies are recorded form data, served at materialization |
| 15 | Precompiling chains | Superseded | Chains are recorded form data |
| 16 | The no-processor body post-state and the bodied unnamed carrier | Superseded | All three `InitSubTemplate` post-states are recorded per body (artifact contract AC-7) |

### The v3 window items

| # | Item | Migration | Where it lands in 3.0.0 |
| --- | --- | --- | --- |
| 1 | `Heddle.Generator` removed; the last 2.x package deprecated on NuGet naming `Heddle.Build` | Reference `Heddle.Build`; keep items, metadata and option properties | CHANGELOG 3.0.0; [precompilation.md § migration note](../../precompilation.md#upgrading-from-version-2) |
...
| 5 | Typed entry points render under process-wide default options and throw `PrecompiledMismatchException` on a failed validation of their own artifact | Assign the default options; `Register` + `ValidateAll` at startup | `PrecompiledTemplates.DefaultOptions` / `BindTyped` ([precompilation.md](../../precompilation.md)) |
| 6 | Engine ids replace build-twin ids; `HED70xx` ids for engine-diagnosed facts retire in place | Re-key `WarningsAsErrors` / suppressions on the engine id | The retired-ids register (R4) in [cross-cutting-decisions.md](cross-cutting-decisions.md#claimed-diagnostic-ids-registry) |
| 7 | Unbindable function calls: bodiless value sites are late-bound data; bodied or hook-typing use is refusal class (c) | `[ExportFunctions]`, else `Precompile="false"` | `HED7014` / `HED7031`; `CorpusIntent` |
...
The 2.1 record's second residue (a change-nothing ruling over `src/Heddle.Performance`) is moot: that
project no longer exists; the .NET benchmark harness is `benchmarks/dotnet`.
...
> **Note (2026-09-18).** The `PrecompiledRuntime.ResolvePartial` removal row was adopted by the
> [3.0 window](#30-window--pending-tag-21-retired-without-a-release) (2.1 item 13 / v3 item 2) and is removed. The rows below that
> were recorded under the 2.x generator program cite seams v3 removed (`PrecompiledRuntime.DynamicMember`,
> the emitter's cast-pin guard, "the generator binds"); their preconditions are re-verified against the
> compiled form before any adoption. The candidates themselves stand.
