...
| F-202 | the `composed-page`/String cell of the data-only compiled-form arm measured slower than the runtime tier, outside error, on repeated samples, while the same workload's Utf8 and TextWriter sinks and the table-on arm were within error and allocation and Gen2 rate were equal or better — neither allocation nor GC | 3 | open — to re-measure at the baseline budget (`bench-techniques`: `TechniquePrecompiledDataOnlyBenchmarks` against `TechniqueRuntimeBenchmarks`); sink-specific, confined to materialising the largest output as a string through the compiled-form adapter; profile that path if it reproduces |
