...
| F-202 | `composed-page`/String, data-only arm, renders 5.5% slower than the runtime tier at the baseline budget (10 launches × 5 iterations: 30.13 ± 0.36 µs runtime, 31.79 ± 0.60 µs precompiled, the third sample in a row outside error); the same workload's Utf8 and TextWriter sinks and the table-on arm are within error, and allocation and Gen2 rate are equal or better, so it is neither allocation nor GC | 3 | open, recorded 2026-09-17 from [docs/benchmarks/2026-09-16](../../benchmarks/2026-09-16/index.md) (`TechniquePrecompiledDataOnlyBenchmarks` against `TechniqueRuntimeBenchmarks`, `bench-techniques`); sink-specific — confined to materialising the largest output as a string through the compiled-form adapter. Chase after v3: profile the String-sink path of the adapter against the runtime tier's; re-measure at the baseline budget before re-reporting |
