# Coding standards (shared across all phases)

Normative for all code written under the phase specs in `docs/spec/`. It has three parts:
the **repository style** (inferred from the codebase, July 2026 — preserved deliberately),
...
run `dotnet format` or IDE whole-file cleanups as part of a phase; keep diffs limited to
...
- `LangVersion` is `latest`: C# 14 language features are usable on **all** TFMs when they
  don't require runtime support. Per the roadmap's .NET 10 review, almost nothing warrants
...
- **Compile-path problems are collected, not thrown.** Anything wrong with a template
  ...
  surfaces raw exceptions for template mistakes. New phases follow this pattern and claim
...
Heddle's benchmark story (faster than Razor, lower allocation) is structural; phase work
must not erode it:

- **No new per-render allocations** on the render path. `Scope` stays a `readonly struct`
  passed by `in`; per-render state that is optional must be lazily created (the phase 3
...
- **Perf claims are proven by BenchmarkDotNet** in
  [src/Heddle.Performance](../../../src/Heddle.Performance), not asserted. Any phase that
...
- Compile-time cost is "compile once, render many" — moderate compile-path allocation is
  ...
  (phase 7's raison d'être) get measured too.
...
- **Additive by default.** New capability arrives as new extensions, new opt-in options
  ...
  Where a phase spec says "existing suite byte-identical", that is the acceptance test of
  this rule.
- **Breaking changes land only in the single 2.0 window**
  ([D2](cross-cutting-decisions.md#d2--the-single-20-breaking-window)) and each must ship
  with a migration note in the phase's docs deliverable.
- **`TemplateOptions` completeness invariant:** every new option property must be added to
  ...
  (`ProvideLanguageFeatures`); phase 2 adds a reflection-based completeness test that
  every later phase inherits for free.
- Public API additions carry XML docs and a same-phase update to the published docs pages
...
1. **Correctness and back-compat** — existing templates render byte-identically unless a
   ratified 2.0 break says otherwise.
...
3. **Simplicity and readability** — the smallest design that meets the phase's success
...
5. **Abstraction and extensibility** — last: a seam is added when a second consumer exists
   or the roadmap ratified one, not because one might appear.
...
- **Open/closed** — the sanctioned open points are the extension registry
  (`[ExtensionName]`), the options object, and the seams named by the roadmap (function
  registry, output profiles, publish/read channel, resolver). Prefer a new extension over
...
- Consolidate when two call sites must never diverge **semantically** — e.g. member-path
  ...
  and be shared by the member tier and the native-expression tier (a phase 1 work item).
...
- Build what the phase's success criteria require, nothing beyond. The roadmap's items
  marked "recorded, not v1" (auto-published outcome records, precompiled replace flags,
  async model resolution, …) stay unbuilt; each phase spec lists them under *Deferred
  items* with a revisit trigger.
- YAGNI does **not** cut: tests, benchmarks, the diagnostic-ID scheme, docs, or the small
  refactors that keep the next phase cheap (Fowler: yagni is only viable when the code
