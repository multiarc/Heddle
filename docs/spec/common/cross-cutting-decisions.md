...
> **Superseded 2026-09-06 (phase 4):** the v3 build printer emits no `u8` literals and the
> `HeddleEmitUtf8Pieces` option is retired (`HED7037`). There is no precompiled-only UTF-8
> emission left to govern; the decision below is history.
...
## Program record — precompilation v2 (closed)
...
- The 2.x generator tier is deleted whole: `src/Heddle.Generator/`, `src/Heddle.Generator.Tests/`,
  `src/Heddle.Generator.IntegrationTests/`, the solution entries, the samples project reference, the
  `.gitattributes` pins, the `Heddle.Generator.*` `InternalsVisibleTo` grants, and the props-file
  commentary it owned. (P4-R1)
- Deleted with it, by the P4-R2 rule (a member whose only caller was generated 2.x code, the 2.x
  generator, or a test of either): `PrecompiledRuntime`, `PrecompiledInitSite`, `PrecompiledInitBody`,
  ...
  `ObserveMode`, `IHeddleTemplateManifest`; the four public `PrecompiledTemplateInfo` constructors,
  `Capabilities`, `LinePathForm`, `InitSites`, `IsPrecompiled` (with its three readers); every
  `PrecompiledSchema` feature-gate constant (`DynamicMemberRouting`, `PropLayoutFingerprint`,
  `PerCarrierLocals`, `RegisteredName`, `LateBoundFunction` schema and arity, the `Emits*` flags,
  `LinePathForm`, `AmbientModelType`) — the compiled-form window itself (`Current`, `Min`/`Max`,
  `CompiledFormSchemaVersion`) stays; the eleven `HeddleBuildOptions` observe/emit members; `PrecompiledBodySupply`,
  `PrecompiledChildSupply`, `PrecompiledSiteFallbackExtension`, the `DefaultFunctionTable` rows; the
  extension/definition/out-parameter precompiled seams and the carrier setters constructor; the gauntlet's
  `!entry.IsPrecompiled` first step; the `HeddleTemplate.Compile(CompileContext)` child-supply probe.
  `Strategy` is internal; `TemplateResolver` holds the internal access. (P4-R2)
...
**Retired-ids register (R4)**

- `HED7005`: unpaired-surrogate warning. No build raises it. (R4-HED7005)
- `HED7006`: unbindable-extension error. No build raises it. (R4-HED7006)
- `HED7007`: unresolvable-model spelling. Retired only as a generator-issued diagnostic; the build
  host raises the same fact through its own channel. (R4-HED7007)
- `HED7008`: unresolvable-member error. No build raises it. (R4-HED7008)
- `HED7011`: import-not-included error. No build raises it — the host reads an import outside the
  item set from disk, mirroring the engine's `ImportMap`, so the fact never fires; an unreadable
  spelling is the engine's own `HED4009`. Retired 2026-09-17. (R4-HED7011)
- `HED7015`: unread-hook warning. No build raises it. (R4-HED7015)
- `HED7016`: branch-role drift warning. No build raises it. (R4-HED7016)
- `HED7017`: malformed-extension-parameter error. No build raises it. (R4-HED7017)
- `HED7019`: unresolved-engine-version warning. No build raises it. (R4-HED7019)
- `HED7022`: unknown-output-profile error. No build raises it. (R4-HED7022)
- `HED7023`: ambiguous-type-name error. No build raises it. (R4-HED7023)
- `HED7024`: private-region-fill error. No build raises it. (R4-HED7024)
- `HED7025`: unbindable-function-call error. No build raises it. (R4-HED7025)
- `HED7030`: inaccessible-model-symbol warning. No build raises it. (R4-HED7030)
- `HED7034`: unobserved-engine note/error. No build raises it. (R4-HED7034)
- Every entry keeps its constant, catalog row, registry row and published row, annotated as retired;
  no entry's number is ever reused. (P4-R6)

**REVISED-RECORDS (phase-4 corrections of earlier records and claims)**

- RR4-1: the phase-3 roundtrip suite's refuse-nothing expectation is corrected — engine-internal
  built-in calls (`upper`, `len`) decline per P3-R2 once the 2.x shim façade is gone, list in `HED7031`,
  rebuild from data at lax load, and never serve under strict load. The printed bytes are unaffected
  (declined sites rebuild identically); only the tier serving them changed.
- RR4-2: the document-shaping generator-driver lockstep is removed with the generator; the runtime
  half survives with a phase-3-era end marker (`FormRecord` capture replacing the removed
  `RuntimeDocument` return).
- RR4-3: `PipelineContractTests` is replaced by `BuildSurfaceContractTests`, which pins the
  `Heddle.Build` surface (props/targets/response-file metadata wiring) instead of the deleted
  analyzer pipeline.
- RR4-4: `HED7007`'s status is precisely "generator-retired, host-raised" — the phase-4 retired list
  names it, and the build host (`heddle compile`) raises the same fact. The registry row, constant and
  catalog row say exactly this; no other retired entry has a live raiser.
...
| `HED7005` | [precompilation.md](../../precompilation.md) | **Retired in place — no build raises it, and the id stays claimed.** It was a generator build-time warning for an unpaired surrogate in static text; the Roslyn analyzer is gone and nothing raises this fact. Constant, catalog row and published row stay; the id is never reused |
| `HED7006` | [precompilation.md](../../precompilation.md) | **Retired in place — no build raises it, and the id stays claimed.** It was a generator build-time error for an extension name resolving to nothing; the Roslyn analyzer is gone and nothing raises this fact. Constant, catalog row and published row stay; the id is never reused |
| `HED7007` | [precompilation.md](../../precompilation.md) | **Retired as a generator-issued diagnostic** — the Roslyn analyzer is gone; the build host raises the same fact (an unresolvable `@model` spelling) through its own channel. Constant, catalog row and published row stay; the id is never reused |
| `HED7008` | [precompilation.md](../../precompilation.md) | **Retired in place — no build raises it, and the id stays claimed.** It was a generator build-time error for an unresolvable member path; the Roslyn analyzer is gone and nothing raises this fact. Constant, catalog row and published row stay; the id is never reused |
| `HED7009`–`HED7010` | [precompilation.md](../../precompilation.md) | Build-tier diagnostics: option parse errors, sanitized-name collisions |
| `HED7011` | [precompilation.md](../../precompilation.md) | **Retired in place — no build raises it, and the id stays claimed.** Retired 2026-09-17. It was the build-tier **error** for an `@<<` import not among the compilation's `.heddle` items. The v3 host resolves an import spelling the way the engine's `ImportMap` does — an item's key first, else the file read from disk under the template root — so an import outside the item set is read, not refused, and the fact this id reported never fires; an unreadable spelling draws the engine's own `HED4009`, forwarded under its id (the `HED7011`/`HED4009` twin noted above). Constant, catalog row and published row stay; the id is never reused (R4-HED7011) |
...
| `HED7020` | [precompilation.md](../../precompilation.md) | Build-host **error** — the host faulted. Two facts under one id: the host threw compiling one template (a host defect, not a template error — that template emits nothing and the pass continues), or the outputs could not be written after every template compiled (no artifact, exit 1). Widened 2026-09-16 to name the second fact; the 2.x reading (a generator defect must surface rather than degrade, code-sharing program phase 5 D12a) is the first fact's ancestry. |
...
| `HED7038` | [precompilation.md](../../precompilation.md) | Build **error** — the `Heddle.Build` targets' intermediate model compile (the one compiler run over the project's own sources that lets a template bind a model type the project itself declares) failed. Raised by the targets *after* the compiler's own errors, which stand unchanged: it attributes them to Heddle's pass, says the project's own compile was not reached, and names the remedies (fix the errors; `Precompile="false"` on every template that binds such a model, itself or through an import; or move the models to a referenced project). The build fails either way — this id adds no degrade path. Claimed by the 3.0 review corrections (2026-09-19) |
...
### 2026-08-09 — `HeddleNodeFallback` keeps its scope; it is a *naming* switch, not an accessor switch

**What changed, with evidence.** `PrecompiledRuntime.MemberAccessor`/`NativeAccessor` were introduced
as a per-node escape: a member path the engine resolves but the consumer's compilation may not *name*
is computed by the engine's own delegate instead of costing the template its tier, and
`HeddleNodeFallback=false` turns that escape off. Binding the extensions made those same two methods
**load-bearing** for a second, unrelated population — every member read inside a body the build emitted
type-agnostically binds through `PrecompiledLateAccessor.Bind`, which calls `NativeAccessor`. The
programme's plan predicted that this would change what the property means, so the property was audited
against the code rather than against the prediction.

It does not change. `GlobalConfig.NodeFallback` is read at exactly two sites in
`TemplateEmitter.cs` (deleted with the 2.x generator in v3) — the member-tier
`EmissionPlanKind.EngineAccessor` selector and `TryNativeAccessorEscape` — and neither is on the
type-agnostic body path, which returns its `PrecompiledLateAccessor` before either is reached.
`HeddleNodeFallback=false` therefore restores the whole-template degrade for an *unnameable member* and
leaves a type-agnostic body precompiling exactly as it does with the property on. Pinned by
`EngineAccessorFallbackTests.WithNodeFallbackOffATypeAgnosticBodyStillPrecompiles`, beside the two
existing opt-out cases that pin the degrade it does restore.

**The decision, and why it is the right scope rather than an accident.** The property is a *naming*
switch: it decides between spelling a member in generated C# and computing it through the engine, and
it exists so a project that would rather see the whole-template degrade than a delegate can ask for it.
Inside a type-agnostic body there is no such choice to make — the build never resolved the model type,
so nothing there could have been spelled, and the accessor is the only emission rather than an escape
from one the compilation refuses. Widening the property to cover it would turn one opt-out into two
different things sharing a name, and would remove a capability (custom extensions precompiling) in the
name of a preference about member spelling. The published description in
[precompilation.md](../../precompilation.md#compile-options-msbuild-properties) states the scope
explicitly for the same reason: the previous wording, "`false` restores the whole-template degrade",
was true when the property had one population and reads as a wider claim now that it has a neighbour.

**What it amends.** The `HeddleNodeFallback` row of breaking-windows item 11, which records the
property as added; nothing about that record moves. No behaviour changes here — this note records what
the code already does, and the test that keeps it doing it.
