...
| `HED2004` | [phase-3-html-context-encoding-lint.md](../post-2.0/phase-3-html-context-encoding-lint.md) | HTML-context encoding lint (`MissingContextEncoder`) — warning; bare `@(value)` in an attribute/`<script>`/URL position under an explicitly declared `Html` profile without the matching `@attr`/`@js`/`@url` encoder |
...
| `HED4005` | [phase-2-authoring-ergonomics.md](../post-2.0/phase-2-authoring-ergonomics.md) | `{{ x }}`-in-text misread lint (`LiquidStyleInterpolationMisread`) — warning; a bare `{{ identifier }}` / `{{ dotted.path }}` in literal text, suggesting `@(…)` |
| `HED4003` | `@import` removal (spec retired — git history) | `@import()` **removal error** — the legacy include is removed; positioned at the call, severity error, naming `@<<`/`@partial`. Repurposed from the improvement-spec B3 deprecation *warning*; the owning spec is retired, so the normative message/trigger/position now live in the [amendments ledger](../records.md#cross-spec-amendments-ledger) (the live pointer) |
...
| `HED7017` | [phase-8-extension-parameters.md](../post-2.0/phase-8-extension-parameters.md) | Generator build-time twin for a malformed extension `[Prop]` parameter declaration (Error), incl. an inherited-`[Prop]` re-declaration widening. Phase 8 reuses call-time `HED5001`–`HED5004` and declaration-side `HED5007`/`HED5008`/`HED5009`/`HED5010`/`HED5015` for extensions, and *additively relaxes* `HED5005` only (no new call-time id) |
