...
| Existing three raw workloads | Unaffected: their outputs never exercised N1's BOM rule (no twin emits one) or N5 (raw suite). **Amended 2026-08-08 ([E20](../../records.md#cross-spec-amendments-ledger)):** `composed-page` was redesigned into a genuine full-page layout workload (live body slot, hybrid blob/structured model, nav rendered from `NavData` through nested partials) and `fragment-heavy` into four dispatched fragment kinds with one nesting level; the corpus rows for workloads 1 and 6 were re-exported (goldens, `.verify.json`, manifest hashes) and the manifest gained a `fixtures` section for `fixtures/composed-page/nav.json`. The CONTRACT itself — N1–N5, N3b, the gates — is untouched; the normalization pipeline is content-agnostic and no consumer's normalizer changes. `trivial-substitution` and `large-loop` remain byte-frozen. |
| Razor twin | Remains outside every gate, as today; not part of the cross-stack contract. **Amended 2026-07-25 ([E5](../../records.md#cross-spec-amendments-ledger)):** superseded. This row recorded that Phase 1 was not touching Razor, not a ruling on where a non-parity row belongs. Razor is now a full `composed-page` parity twin, authored against the shared `TwinContent` fixtures and asserted by `ParityCheck` like the Liquid/Scriban/Handlebars twins; the old full-page `Views/layout.cshtml` workload and its duplicated fixture tree are removed. It remains outside the *cross-stack* contract (phases 2–6 do not measure it). |
