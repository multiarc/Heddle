...
| Composed-page model source | *(read)* `TwinContent.cs` — `SectionMeta/SectionSocial/SectionPageScripts/SectionEndPageScripts` consts, `Comp*` consts, `AreaOrder`, `Areas` (delegates to `AreaComponent.Areas`), `Sections()`/`Components()` dictionaries — the literals the JS model transcribes |
