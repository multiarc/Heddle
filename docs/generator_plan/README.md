...
| 7 | [shared-test-corpus](phase-7-shared-test-corpus.md) | 0 (posture + corpus sweep); 1–6 landed (their byte-neutrality gates are the invariant) | Extend the program's single-artifact principle from production rules to **test inputs** — one shared, intent-declaring template corpus consumed by every tier's tests (source templates *and* goldens), so a shape exists exactly once, both tiers compile the same bytes, and phase 0's D4 residue closes | **stage 0 implemented (2026-07-26); stages 1–5 stopped with cause.** The sharing mechanism, the shared accessor, the intent table and set-equality gating are landed and green (+7 / +1 tests); the inline-string backfill is **not**, because stage 1's "18 character-for-character duplicates" agree only as *substrings* — the whole templates differ by model type and preamble, and unifying them needs an unscoped cross-project fixture-model merge (**Q8.42**) |
...
7. **Coverage narrower than claimed** — phase 0's resolver byte-parity covers 9 templates, not the
   ...
   **Owned by [phase 7](phase-7-shared-test-corpus.md)**: the floor, the traversal and the five
   ...
   templates to **32**. The inline-string residue is **not** closed — its migration stages are stopped
   with cause (**Q8.42**), so ~130 feature tests still never cross the gauntlet.
