...
- Additive API on the renderer for allocation‑free writes where callers have them:
  ...
  ints/dates/decimals — the formatter extensions (`int`, `date`, `money`) migrate to these
  opportunistically).
...
- **Pre‑encoded static text**: `RuntimeDocument`'s document pieces get lazily cached UTF‑8
  ...
  output bytes are then a straight `memcpy` into the buffer writer.
...
1. Should formatter built‑ins (`int`, `date`, `money`) migrate to span formatting in this
   phase or opportunistically after? (Lean: after — keep the phase focused on the sink.)
2. Expose the pre‑encoded‑UTF‑8 cache size / eviction as an option, or unbounded per
   compiled document? (Lean: unbounded — it is proportional to template size, not data.)
