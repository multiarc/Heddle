...
> Depends on: nothing. Enables: sandboxing, cheaper compiles, and a 1:1 emission path for
> phase 7 (build‑time template pre‑compilation).
...
longer imply arbitrary code), and removes Roslyn from the common compile path (startup cost).
It also hands the build‑time pre‑compilation backend (phase 7) a 1:1 expression‑to‑C#
emission path — a soft prerequisite, not a hard one.
...
standard escapes, `'c'` char, `true` / `false` / `null`. Char literals are **in scope for
v1** (decided July 2026) — the token is trivial and completes the C# literal set.
...
- **`format` supports composite formatting from v1** (decided July 2026): both the
  single‑value form `format(X, "d")` (→ `IFormattable.ToString(format,
  InvariantCulture)`) and the composite form `format("{0:d} — {1}", X, Y)` (→
  `string.Format(InvariantCulture, …)`, variadic). The format string must be an expression
  yielding a string (typically a literal); malformed composite strings surface as the
  exception‑safe empty result the registry's built‑ins already guarantee, with the
  positioned compile error reserved for arity mismatches that are statically visible
  (literal format string with more `{n}` slots than arguments).
...
### Documentation (same phase) — a dedicated native‑expressions reference

**Decided (July 2026): native expressions get their own section in the published docs**
(a new `native-expressions.md` page or a top‑level section in
[language-reference.md](../language-reference.md) — pick at implementation by size). It is
the authoritative "what native expressions do" reference: the operator/precedence table
above (whose semantics are pinned by the operator‑semantics test table — the docs and the
tests must state the same rules), the literal set, the `.`‑hops‑are‑null‑safe equivalence
(why there is no `?.`), the registered‑function model and default registry, and
`ExpressionMode` with the sandbox trust boundary. Cross‑linked from
[built-in-extensions.md](../built-in-extensions.md) (`@if`, `@for` parameter docs) and the
C#‑tier docs (when to escalate to `@` expressions).
...
New code dominates (AST, `NativeExpressionCompiler`, `FunctionRegistry`) — additive risk.
Three genuinely shared touch points are where regressions can hide:

1. **The grammar** — new CALL‑mode tokens and a 4th `call` alternative change ALL(\*)
   prediction paths for *every* template, including ones that never use expressions.
2. **The extracted property‑resolution helper** — the member tier re‑routes through it, so
   any semantic drift (null‑safe hops, `[Hidden]`, error wording) hits every existing
   member path in every template.
3. **The `TemplateFactory.Create` failure path** (function fallback) and the
   `TemplateOptions` plumbing (`ExpressionMode`, `Functions`, copy‑ctor).

Plus mechanical churn under `generated/` that must stay reviewable in isolation.

### Regression requirements

- The full xUnit suite (`src/Heddle.Tests`) + golden corpus **byte‑identical** under the
  `Native` default — the primary gate (success criterion 2).
- A member‑tier equivalence run after the helper extraction: every existing member‑path
  fixture produces identical output **and identical error messages**.
- `ExpressionMode.MemberPathsOnly` reproduces today's errors for every expression fixture
  (criterion 3).
- Parse‑time benchmark of the existing fixture corpus before/after the grammar change
  (the ALL(\*) prediction‑cost risk) via `Heddle.Performance`.
- The ANTLR regen lands as a dedicated commit, reviewed standalone.

### TDD verdict

**Yes — this phase is the roadmap's best TDD fit.**

- The operator‑semantics table *is* a ready‑made TDD table: write the C#‑computed expected
  values first (the same table the docs section publishes), implement
  `NativeExpressionCompiler` until green. Spec, tests, and docs are one artifact.
- **Security tests first**: the sandbox‑negative suite (method calls, unregistered
  functions, assignment, dynamic‑scope operators) is written before the compiler features
  it constrains — the sandbox must fail closed from the first commit.
- Grammar work is TDD‑adapted rather than classic red/green: a token/parse fixture corpus
  (expression string → expected AST or positioned error) is the executable spec each
  ANTLR regen is validated against.

### The verification loop

1. Extend the spec first: new rows in the operator table (C#‑computed expectations), new
   parse‑corpus entries, new sandbox negatives — all failing.
2. Grammar step: lexer/parser change → regen → parse corpus green → existing corpus
   parse‑time benchmark within budget.
3. Compiler step: implement in `NativeExpressionCompiler` until the operator table and
   negatives are green.
4. Gate: full suite + goldens byte‑identical, member‑tier equivalence run,
   `MemberPathsOnly` restores today's errors.
5. Red → fix code. The operator table is never hand‑edited to pass — its expectations are
   C#‑derived; a disagreement means re‑deriving from a compiled C# reference, which is a
   spec event, not a fix.
6. Exit: success criteria 1–6 green in one combined run.

### The final suite (including deferred phase 9 items)

- Table‑driven operator/promotion tests (authoritative — the docs decision makes this
  table the published reference, so tests and docs cannot drift apart).
- The sandbox‑negative suite (permanent security regression tests), the parse corpus,
  registry/overload‑binding tests (incl. composite `format`), and the unchanged goldens.
- **Deferred to [phase 9](phase-9-demo-and-integration.md):** the
  ...
  boundary end‑to‑end, golden‑asserted in CI; and the demo page's in‑browser rendering,
  which is a standing integration proof of the Roslyn‑free path (the WASM bundle cannot
  carry Roslyn, so if it renders, the sandbox tier really is self‑sufficient).
...
| `FunctionRegistry` + built‑ins (incl. composite `format`) + standalone‑call unification | M |
| `ExpressionMode` options + copy‑ctor | S |
| Tests (goldens, promotion table, negatives, sandbox suite) | M |
| Docs: dedicated native‑expressions reference section | S |
...
- **Interpreter‑safe built‑in binding (forward‑looking)** — mechanism: the
  `System.Linq.Expressions` *interpreter* (`Compile(preferInterpretation: true)`; note the
  engine never targets Native AOT — phase 7's ratified non‑goal — so this is pure
  future‑proofing) throws on expressions calling methods with
...
None — all resolved July 2026 and integrated above: composite formatting ships in v1
(see the `format` bullet under the function registry), char literals are in scope for v1
(Literals), and native expressions get a dedicated published‑docs reference section
(Documentation).
