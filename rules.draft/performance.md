...
- Perf claims are proven by BenchmarkDotNet (`src/Heddle.Performance`), never asserted. Hot-path changes run affected benchmarks before/after on the same machine: allocated bytes must not increase; mean must stay within reported error. Intentional trade-offs need maintainer ratification.
