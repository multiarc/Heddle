...
/**
 * large-loop (raw suite) - WI5 benchmark class. Annotations are the spec D9 pin: JMH
...
public class LargeLoopBench {

    private static final String WORKLOAD = "large-loop";

    private Object model;

    @Setup(Level.Trial)
    public void setup() {
        model = Models.LOOP_ROWS;
    ...
    private String renderJteControlled() {
        return JteEngines.renderPlain("controlled/large-loop.jte", model);
    }

    private String renderJteIdiomatic() {
        return JteEngines.renderPlain("idiomatic/large-loop.jte", model);
    }
