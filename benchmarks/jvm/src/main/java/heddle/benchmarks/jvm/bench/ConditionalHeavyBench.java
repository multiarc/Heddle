...
/**
 * conditional-heavy (raw suite) - WI5 benchmark class. Annotations are the spec D9 pin: JMH
...
public class ConditionalHeavyBench {
    ...
    private static final String WORKLOAD = "conditional-heavy";

    private Object model;

    @Setup(Level.Trial)
    public void setup() {
        model = Models.CONDITIONAL_ROWS;
    ...
    private String renderJteControlled() {
        return JteEngines.renderPlain("controlled/conditional-heavy.jte", model);
    }

    private String renderJteIdiomatic() {
        return JteEngines.renderPlain("idiomatic/conditional-heavy.jte", model);
    }
