...
/**
 * fortunes-encoded (encoded suite) - WI5 benchmark class. Annotations are the spec D9 pin: JMH
...
public class FortunesEncodedBench {

    private static final String WORKLOAD = "fortunes-encoded";

    private Object model;

    @Setup(Level.Trial)
    public void setup() {
        model = Models.FORTUNE_ROWS;
    ...
    private String renderJteControlled() {
        return JteEngines.renderHtmlControlled("controlled/fortunes-encoded.jte", model);
    }

    private String renderJteIdiomatic() {
        return JteEngines.renderHtmlIdiomatic("idiomatic/fortunes-encoded.jte", model);
    }
