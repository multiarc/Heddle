...
/**
 * encoded-loop (encoded suite) - WI5 benchmark class. Annotations are the spec D9 pin: JMH
...
public class EncodedLoopBench {

    private static final String WORKLOAD = "encoded-loop";

    private Object model;

    @Setup(Level.Trial)
    public void setup() {
        model = Models.ENCODED_ITEMS;
    ...
    private String renderJteControlled() {
        return JteEngines.renderHtmlControlled("controlled/encoded-loop.jte", model);
    }

    private String renderJteIdiomatic() {
        return JteEngines.renderHtmlIdiomatic("idiomatic/encoded-loop.jte", model);
    }
