...
Environment documentation for the Linux cross-check (phase 8 of the program). It pins the
dual-boot establishment rules (what is specified vs what is recorded), the closed
environment-record checklist (firmware / CPU-mitigation / frequency-policy items and the
per-engine toolchain/runtime/libc/allocator identity list), the one-machine-state rule for the
Linux session, and the version-pinning-and-delta procedure against each Windows source run.
Decisions of record: D1–D5 (this document carries their normative text).
...
1. **Distribution and image.** Ubuntu **Server** 24.04 LTS, the **latest 24.04.x point release**
   available at establishment time, minimal server install, no desktop environment. Rationale
   (D1): a desktop session adds compositor/indexing/update services that are noise sources,
...
6. **Boot-parameter configuration.** One additional GRUB entry ("Ubuntu — benchmark isolation")
   carries the isolation parameters of D2:
...
the run log committed with the report (paths per D18: the `benchmarks/linux-crosscheck/` tooling
directory and the published report's `environment/` capture). This document deliberately does not
...
5. All harness runs execute in this state (order per the
   [measurement-run matrix](linux-harness-settings-and-validation.md#measurement-runs-d13)).
   **No per-harness priority
...
`capture-environment.sh` (committed under `benchmarks/linux-crosscheck/`; D18) collects
**exactly** the following, and the report's environment block summarizes it.
...
procedure of the Python harness ([python-harness.md](python-harness.md#run-protocol)) is
unchanged.
