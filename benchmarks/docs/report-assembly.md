...
Everything here
is **normative** for the consolidated report (phase 7): the published directory's contents, the exact `index.md` structure, table
...
the publication checklist. Decision rationale (phase 7 D1–D14) is condensed in the program
...
Workload order everywhere — **amended 2026-07-25 by amendment E7**:
...
2. **Intro** (no heading) — required statements, each present:
   ...
   - links to the metrics & publication protocol and parity contract v2 (as published paths
     from the report: `../../../benchmarks/docs/...`);
...
5. **`## How to read these tables`** — required content:
   ...
   - the **ranking-scope rule** (as amended 2026-07-25 by the phase 7 D6 amendment):
...
10. **`## Per-ecosystem sidebars — not cross-comparable`** — one `###` per included
    ecosystem, contents per phase 7 D7; every sidebar containing allocation/memory data opens
...
| Reason (absent/out-of-scope only) | closed vocabulary: `phase cut` · `run not yet published` · `run not protocol-conformant (R1)` · for PHP/Ruby: the recorded program-level non-goal rationale (phase 1 plan, non-goals) |
...
Primary source (phase 7 D4): the published source `index.md` tables. The raw artifacts below
...
[workloads.md](workloads.md#the-set-at-a-glance):
...
- **Generate mode (default):**
  ...
  4. emit `consolidated-tables.md`: all D5 wall-time tables (workload-major, ecosystem-minor,
     ...
     **Amended 2026-07-25 (amendment E7):** workload-major order is the tier order defined under
  ...
  4b. the **implied-throughput numerator is the rendered output size, not the golden
     `byteLength`** (amendment E7). The golden oracle is stored normalized —
...
1 reference run(s)). Evaluate each pair against R1–R5 (phase 7 D11):
...
`rerun-requests.md` beside this document (would be created on first need; it has never been
needed). Entry format (append-only table):
