...
Companion document of the JS harness (`benchmarks/js/`). It pins the harness project
...
[js-templates-and-models.md](js-templates-and-models.md); the contract and protocol being implemented
are Phase 1's [parity-contract-v2.md](parity-contract-v2.md)
and [metrics-protocol.md](metrics-protocol.md).
...
benchmarks/js/
  ...
  README.md                    ← pointer to this document + reproduce commands
  ...
  src/
    models/<id>.mjs            ← 8 frozen models (js-templates-and-models.md)
    templates/
...
- **Flags:** every measurement invocation uses `--expose-gc --allow-natives-syntax` (mitata's
  ...
  amended 2026-07-25, E4 — the `%FlattenString`
...
- **Passes (amendment E6, resized by E14):** each render track is measured as **38 repeat
  passes** at the `short` budget (114 at `baseline`), one process
  each, and `bench/aggregate.mjs` publishes the median of the per-pass `avg` with the cross-pass
  `min … max` as dispersion. Because the per-cell budget cannot be raised, the leg's
  per-engine ~10 min budget (E14) is spent on independent processes — the cross-process term a
  single-process harness otherwise never samples. `cold-compile` stays one pass (compile-dominated
  D10 sidebar).
- **Statistic:** the published wall time per render is mitata's `avg`, per the wall-time
  statistic mapping (Q2.1) in [metrics-protocol.md](metrics-protocol.md#wall-time-statistic-mapping-q21);
  dispersion published alongside is the
...
- **MATERIALISATION-CHECK (D12 companion, E4):**
...
**Procedure.** Since amendment E6 this is not a
...
1. `./run.ps1 bench/<track>.mjs -Repeat 38` (`--repeat 38` on Linux; 114 at `baseline` —
   E14) — N consecutive full runs
   of the track; captures land in `artifacts/stability/<track>/run-1..N.{txt,json}`. The
...
The stability captures and the summary ship with the published report (immutable
evidence that stability was verified before publication).
...
4. **`## Results — what this run actually shows`** — in order:
   ...
   - **Helper disclosure text** (js-templates-and-models.md verbatim) in the controlled
     narrative.
   - **Prose callouts** per honest-reporting rule 2: every workload
