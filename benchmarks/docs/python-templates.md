...
Template documentation for the Python leg of the program (phase 5). It pins the exact template
texts (controlled track) and constructs-plus-citations (idiomatic track) for Jinja2 3.1.6 and
Mako 1.3.12 across the eight workloads. Workload models and oracle characteristics are
defined once in [workloads.md](workloads.md) and are
not restated; the gates are defined in the
[parity contract v2](parity-contract-v2.md). Authoring-style
rules are decision D3, engine escaping configuration D4/D5 — condensed below.

Template texts below are normative: the committed template files transcribe them.
...
## Decisions of record (condensed — cited by `runner/engines.py`)

**D3 — controlled-track authoring rules, one style per engine.** Jinja2: all control tags
(`{% %}`) and expressions written inline in the literal HTML; line breaks appear only in
inter-tag positions (between a `>` and the next `<`); the environment pins the library
whitespace defaults explicitly in code (`trim_blocks=False`, `lstrip_blocks=False`,
`keep_trailing_newline=False`); the `{%- -%}` / `{%+ +%}` markers are never used. Mako: `%`
control lines for all control flow (each on its own line — they contribute zero output bytes);
literal lines end with a trailing `\` wherever the line break would otherwise land inside
element text; `<%include>`/`<%page>`/`<%def>` tags are followed by `\` when the byte after them
must not be a newline (these tags do not consume their trailing newline); `<% %>` Python blocks
are not used in controlled templates. Both: template files are committed LF
(`.gitattributes`: `benchmarks/python/templates/** text eol=lf` — Mako passes a literal `\r\n`
through to output).

**D4 — raw suites run untouched non-escaping defaults; encoded suite pins one engine-level
escaping configuration each.** Raw (workloads 1–6, both tracks): Jinja2 a plain
`Environment(autoescape=False)` (the standalone default, pinned explicitly in code); Mako a
`TemplateLookup` with `default_filters` left at its `None` default (internally `["str"]`); no
escape-bypass syntax (`|safe`, `Markup`, `${x | n}`) appears in any raw template. Encoded
(workloads 7–8): a separate `Environment(autoescape=True)` and a separate
`TemplateLookup(default_filters=["h"])` on the controlled track — engine-level configurations
of the engines' own documented escaping paths; no per-expression escape filter appears in
controlled encoded templates. Neither engine offers entity-spelling configuration, so the gate
runner implements the contract's full
[N5 table](parity-contract-v2.md#entity-canonicalization-n5) for encoded candidates (the only
rewrite these engines actually trigger is MarkupSafe's `&#34;` → `&quot;`).

**D5 — idiomatic-track constructs.** One shared long-lived engine object per engine: Jinja2 one
`Environment` with `FileSystemLoader` and `select_autoescape()` (encoded templates named
`<id>.html` so autoescaping engages by extension, raw templates `.jinja` so it does not); Mako
one `TemplateLookup`, with the encoded templates carrying `<%page expression_filter="h"/>`.
Layout-shaped workloads use each engine's inheritance mechanism (`{% extends %}` /
`<%inherit>`); repeated fragments use the documented reusable unit (Jinja2 macro via
`{% from … import %}`; Mako `<%def>` via `<%namespace>`). Whitespace is authored naturally —
the idiomatic gate is the functional verifier, which normalizes. The per-workload doc
citations are in the [idiomatic track](#idiomatic-track) table and in every template file's
header comment.
...
All template files are committed LF (`.gitattributes` rule — D3 above).
Controlled and idiomatic trees never share files. Context variable names (from
`runner/data.py`, [D6 — condensed in python-harness.md](python-harness.md#model-data-d6-condensed)) —
identical for both engines and both tracks:

| Workload | Context |
|---|---|
| composed-page | `nav` and NOTHING else (amendment E20/E22) — the structured navigation model, loaded by `runner/data.py` **once at import** from the corpus fixture `GoldenCorpus/fixtures/composed-page/nav.json` (strict UTF-8, `json.load`) via the gate's corpus-dir resolution (`gates.CORPUS_DIR`). Schema (snake_case): `{menus: [{tabs: [{label, href, css, has_dropdown, dropdown_css, columns: [{sections: [{title, href, title_linked, links: [{label, href}]}]}]}]}], footer_columns: [<column shape>]}`. The former `section`/`comp`/`areas`/`area_names` blob dicts are DELETED (E22: no text blobs in the model tier — ALL chrome/fragment text lives in the templates); `TwinContent.cs`/`AreaData.cs`, which they transcribed, no longer exist |
| trivial-substitution | top-level scalars `title, sku, price, brand, category, availability, url, image_url, summary, rating` — values from [SubstitutionContent.cs](../dotnet/src/Models/SubstitutionContent.cs) |
...
Per E21/E22 (the data-not-display rule of the amendments ledger),
...
[trivial-substitution.liquid](../dotnet/templates/controlled/liquid/trivial-substitution.liquid)
...
[large-loop.fluid.liquid](../dotnet/templates/controlled/liquid/large-loop.fluid.liquid)
text — the .NET Liquid entry, dialect-suffixed, identical in both dialect copies —
itself valid Jinja2):
...
[workloads.md](workloads.md#workload-4--mixed-page-raw) line
...
[workloads.md](workloads.md#workload-4--mixed-page-raw)
