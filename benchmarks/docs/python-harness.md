...
Harness documentation for the Python leg of the program (phase 5). It pins the harness layout,
the gate-runner mechanics, the exact measurement CLI (timing, memory, cold compile), and the
Python instantiation of the program's
[metrics & publication protocol](metrics-protocol.md).
Template texts are in [python-templates.md](python-templates.md); of the phase's decisions
D1–D13, the ones the harness code cites are condensed here (D6, D10) and in
python-templates.md (D3, D4, D5).
...
### Model data (D6, condensed)

`runner/data.py` constructs all eight workloads' models as plain `dict`s with the pinned
snake_case keys, module-level (materialized once per process): workloads 3–8 from the exact
generation formulas pinned in [workloads.md](workloads.md); `trivial-substitution` transcribed
from the C# source
([SubstitutionContent.cs](../dotnet/src/Models/SubstitutionContent.cs)); `composed-page`
carries only the structured `nav` model, loaded once at import from the corpus fixture
`GoldenCorpus/fixtures/composed-page/nav.json`. No numeric formatting exists anywhere: the
numeric fields are ints rendered via `str(int)` (invariant by construction), and the one
field that looks numeric but isn't — `rating` — is the pinned string literal `"4.8"`, never a
formatted float. Transcription is never trusted: the controlled byte gate is the transcription
check — any transcription error fails loudly before any timing.
...
6. **N5** (encoded suites only) — one left-to-right scan, non-overlapping, output never
   ...
   `&amp; &lt; &gt; &quot; &#39;` per the
   [contract table](parity-contract-v2.md#entity-canonicalization-n5).
   Regex, verbatim:
...
Failure anywhere ⇒ `SystemExit(1)` with the harness's pinned error-surface message
(`GATE FAIL <engine>/<track>/<workload>: first diff at byte <i> …` + excerpt); in a bench
script this happens before any `bench_func` registration, so pyperf emits nothing.
...
**D10 (condensed):** the cold pass measures cold template construction only — Jinja2
`Environment.from_string(source)` (parse + compile to Python bytecode, no bytecode cache
configured) and Mako `Template(text=source)` (parse + compile to a Python module, in-memory,
no `module_directory`) — each on the controlled-track source text. Disk-cache-inclusive
variants were rejected (they measure deployment configuration, not the engine). The figures
are per-ecosystem and non-comparable (Q1.3), published in their own labeled section, never
beside the Heddle row.
...
*(Verify before any protocol run: `--affinity=4` per the D8 CPU-topology note — confirm the
SMT-pair enumeration with Sysinternals coreinfo and record the confirmed CPU id in the
...
4. **`## Results — what this run actually shows`** — narrative + tables:
   ...
   - **memory** subsection (Python-only): the Q5.1 table (column header "allocated bytes per
     ...
     numbers."*;
   - the exclusion-rationale statement (Django templates, legacy engines, native-extension
     bindings — the phase 5 plan's non-goals / scope-boundary rationale, linked from the
     published report);
...
(if any materialize) print `excluded — documented evidence` with a link. The phase's
testing-plan report checklist was the sign-off instrument for this section.
