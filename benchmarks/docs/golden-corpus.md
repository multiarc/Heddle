...
Specifies
the on-disk corpus the phase 2–6 harnesses consume as their parity reference: location,
per-entry format and metadata, the export tool, the verification commands, and the
...
benchmarks/dotnet/GoldenCorpus/
  ...
  fixtures/
    composed-page/
      nav.json                   ← structured nav model (amendment E20) — the single source the
...
the class itself was retired with the twin text fixtures under E22 — the pipeline lives in the
harness's `src/Gate/Normalize.cs`, unchanged)
...
Git handling: a `.gitattributes` rule pins
`benchmarks/dotnet/GoldenCorpus/*.golden.html -text` (and `*.verify.json` / `manifest.json`
as `text eol=lf`), so no checkout smudging can
alter the golden bytes (per the
[testing-standards line-ending rule](../../docs/spec/common/testing-standards.md#fixtures-and-goldens);
...
|---|---|---|---|
| composed-page *(amended E20 — full-page shape; E22 — chrome needles are literals)* | the `<title>Title</title>` section-default literal → 1; `<li class="nav-link"><a href="` → the total `NavLink` count walked from `NavData`; `<div class="nav-column">` → the total column count (mega menus + footer); `<span class="nav-title">Need Help?</span>` → 1 (the first footer column's title); the full rendered link for the `/content/privacy-policy` NavLink → 1 *(nav needles and counts are computed in `VerifierDefinitions.cs` from `NavData` — the very model every engine renders from — and resolved into the exported JSON; the chrome anchors are literal needles, since all chrome text lives in the templates and is policed by the byte gate)* | in document order: `<!DOCTYPE html>`, `<title>Title</title>`, the alert-fragment anchor (`xmas-shipping-alert-1.jpg`), the header-chrome anchor `id="search-area-form"`, the two secondary-menu fragment anchors, `>Shop All Products</a>` (first mega-tab label), the wholesale-only anchor `/product/coming-soon-paleo-pork`, the retail-only anchor `/products/paleo-friendly-pork`, the slider anchor `homebtmbanners/gluten-hp.jpg` (the spliced body), the footer-chrome anchor `/Assets/images/seeourcatalog.jpg`, the closing `<script src="/bodyend.js"></script>`, `</html>` | — |
...
drift risk this accepts is a recorded user ruling (Q1.5);
