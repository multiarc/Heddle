...
cross-stack set: the three retained anchors and the five workloads phase 1 added. The
...
> [Workload 1 — composed-page](#workload-1--composed-page-raw--amended-e20-e22) below); its
...
([composed-page.heddle](../dotnet/templates/controlled/heddle/composed-page.heddle),
[layout.heddle](../dotnet/templates/controlled/heddle/shared/layout.heddle),
[trivial-substitution.heddle](../dotnet/templates/controlled/heddle/trivial-substitution.heddle),
[large-loop.heddle](../dotnet/templates/controlled/heddle/large-loop.heddle)), their
runner classes, their models, and their benchmark classes stay untouched, so historical numbers
remain comparable run-to-run. Phase 1 added a corpus export and verifier definition for each —
nothing else.
...
- **`composed-page`** — `composed-page.heddle` extends `layout.heddle` via `@<<{{shared/layout.heddle}}`; the
  ...
  [Runners README](../dotnet/GoldenCorpus/README.md) are carried **verbatim**
  into the corpus documentation (decision phase 1 D5,
...
2. **Encoded suite renders every engine's escaping path** (Heddle `OutputProfile.Html` with
   ...
   `FiveEntityTextEncoder` — the `ITextEncoder` that escapes exactly `& < > " '` to the
   canonical spellings and passes everything else through; phase 1 D3).
...
5. **Per-engine model views are materialized once** (static fields), never per benchmark op —
   ...
   `LoopContent` ([LoopContent.cs](../dotnet/src/Models/LoopContent.cs)).
...
nothing per N3b, leading/trailing trim) are tolerated between the text here and the
...
### Model — pure structured data (amendment E20; E22 removed the text half)
...
| JVM (Thymeleaf) | parameterized fragment `layout(~{:: slider-body})` + `th:replace="${bodySlot}"` — feasibility re-run completed 2026-08-08: ALL controlled cells pass the byte gate, no exclusion invoked (new authoring classes B10 fragment-selector tag-name collision and B2b `th:attr` append-at-end order recorded in [jvm-thymeleaf-feasibility.md](jvm-thymeleaf-feasibility.md)) | `th:each` + fragments |
...
| Go (templ) | `@layout(m) { children }` — landed as real children composition (`{ children... }` at the slot), with the void-element `templ.Raw` accommodation documented in [go-port-mapping.md](go-port-mapping.md#controlled-track--templ) | nav components |
...
- Scriban: same structure with `{{ if r.is_bronze }} … {{ else if r.is_silver }} … {{ else }} …
  {{ end }}` (chained `else if` verified against Scriban 7.2.5 — executed probe E).
...
([language-reference — passing the current value into a call](../../docs/language-reference.md#passing-the-current-value-into-a-call))
...
| Handlebars | `<tr><td>{{id}}</td><td>{{message}}</td></tr>` — double-mustache, under `FiveEntityTextEncoder` (phase 1 D3) |
...
- Normalized size ≈ 1 MB (the corpus entry is the largest committed golden; the size is accepted
  — phase 1 §performance considerations: the entry and its per-render output are the program's
  largest artifacts, and allocation columns reflect the output size for every engine equally).
