...
Companion document of the Rust harness (`benchmarks/rust/`). It pins the normative template
...
[workloads.md](workloads.md) (exact strings and generation
...
[parity-contract-v2.md](parity-contract-v2.md); which gate
...
- **Controlled texts are normative the same way Phase 1's twin texts are**: whitespace erased by
  the contract's N2–N4 (line endings, inter-tag runs, edge trim) is tolerated between this document
...
- Where one template text serves both engines it is shown once and marked **(both engines)** —
  ...
  surface for every construct these workloads need (README D7).
...
- File paths are relative to `benchmarks/rust/` (README D2).
...
C# invariant `int` formatting). Per amendment E21/E22
...
| `fortunes-encoded` | Exactly the 12 pinned `(id, message)` rows of Phase 1 [workloads.md — workload 7](workloads.md#workload-7--fortunes-encoded-encoded), transcribed as Rust string literals byte-for-byte (row 4/8 em dashes are U+2014; row 11 is the XSS payload; row 12 the Japanese string) |
...
counts and distinctive values (README testing plan); the composed-page
...
E22 the per-ecosystem blob fixture files and
...
[`trivial-substitution.heddle`](../dotnet/templates/controlled/heddle/trivial-substitution.heddle)):
...
`& < > " '`, so escaping is byte-neutral; README D3).
...
(Mirrors [`large-loop.heddle`](../dotnet/templates/controlled/heddle/large-loop.heddle);
...
[golden-corpus.md](golden-corpus.md#location-and-layout)),
...
## Budget and measurement rules (D9 as amended by E14; WI10)

Folded from the phase README — `benchmarks/rust/benches/*.rs` and the run-all scripts cite
D9/WI10 for these rules.

- **Criterion configuration (D9, as amended by amendment E14 — per-engine measurement budget).**
  Every timed bench target constructs one shared config in its custom `main`
  (`harness = false`):
  `Criterion::default().warm_up_time(Duration::from_secs(5)).measurement_time(Duration::from_secs(26)).sample_size(100).confidence_level(0.95).configure_from_args()`.
  D9's original shape was defaults except `measurement_time` 5 s → 10 s; E14 resized every
  leg to the per-engine budget unit — an engine is **16 cells** (eight workloads × two
  tracks) at ~10 min `short` / ~30 min `baseline` — which for Rust means warm-up 3 s → 5 s
  and measurement 10 s → 26 s **in the committed source** (the `short` profile; a bare
  `cargo bench` is the budgeted regime), with `baseline` layered on via CLI:
  `--warm-up-time 10 --measurement-time 84` (what `run-all.sh --budget baseline` passes).
- **The trailing `.configure_from_args()` is mandatory and must be last.** With
  `harness = false`, Criterion parses the CLI arguments after `--` (`--noplot`, `--test`,
  `--save-baseline`, name filters, the baseline time overrides above) *only* when the custom
  `main` calls it; layered last, the pinned settings survive when no flag overrides them and
  a supplied flag wins. Without it, `cargo bench -- --noplot` and `cargo bench -- --test`
  are silent no-ops. Runs are invoked `cargo bench -- --noplot`.
- **Naming and statistic.** Groups `controlled-<workload-id>` / `idiomatic-<workload-id>`,
  functions `askama` / `tera`; artifacts land at
  `target/criterion/<group>/<fn>/new/estimates.json`. The published statistic per cell is
  `mean.point_estimate` with `mean.confidence_interval.{lower_bound,upper_bound}` (95%) from
  that file — the wall-time mapping of [metrics-protocol.md](metrics-protocol.md#wall-time-statistic-mapping-q21).
  Timed closures are `b.iter(|| runner.render())` where `render()` returns the output
  `String` (drop cost is inside the measurement for every engine equally). No
  process-priority manipulation and no core pinning; dispersion is published so instability
  is visible. Revisit trigger: any cell's published 95% CI half-width exceeding 5% of its
  mean → re-run the full suite with priority elevation as one recorded environment change.
- **Measurement run and report (WI10).** On the protocol machine, in order:
  `cargo run --release --bin gate` (capture output) → `cargo bench -- --noplot` (capture
  ...
  (`Cargo.toml` `=` pins + `rust-toolchain.toml`).
