...
|---|---|---|---|
| .NET (phase 1) | BenchmarkDotNet (`0.15.8` on net10.0 — the protocol-run TFM), `[ShortRunJob]` since amendment E6 | the `Mean` column | `Error` and `StdDev` columns as reported |
...
## The protocol's first exercise (phase 1)

Phase 1 closed by executing this protocol once, intra-.NET, over all eight workloads:

- Suites: `TextRenderBenchmarks` (composed-page), `SubstitutionRenderBenchmarks`,
  `LoopRenderBenchmarks`, plus the five phase-1 benchmark classes
