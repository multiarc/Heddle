...
Companion document of the JS harness (`benchmarks/js/`). It pins the JS model
...
[workloads.md](workloads.md); the gates that judge these texts
are in [parity-contract-v2.md](parity-contract-v2.md).
...
3. **Generation formulas are transcribed, not their outputs** — e.g. mixed-page products are
   ...
   **The models carry DATA only — no derived display strings (amendment E21).** `sku_number`,
...
4. **Pinned literal sets are transcribed verbatim:** the 12 fortunes rows (ids 1–12, messages
   ...
   [SubstitutionContent.cs](../dotnet/src/Models/SubstitutionContent.cs)
   — `Heddle Handbook`, `HB-2001`, `4.8`, etc.); the mixed-page page scalars (workloads.md).
5. **Composed-page is pure structured data — `{ nav }` and NOTHING else (amendment E20, E22).**
...
(The `<style>` ellipsis above is presentational in this document only — the checked-in file
