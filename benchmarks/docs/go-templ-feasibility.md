...
Feasibility record for the Go leg of the program (phase 6). It answers the program's flagged
...
time?** The answer is derived per workload against the pinned template shapes
([workloads.md](workloads.md)) and the normalization pipeline
([parity-contract-v2.md](parity-contract-v2.md#normalization-pipeline)).
...
was no longer a controlled-track risk. Verification spike **S1** — executed as the first
implementation work item; results recorded in `benchmarks/go/README.md` — confirmed the analysis
and pinned templ's actual escaper bytes: the expected clean pass held, and the single
**non-whitespace** contingency (an escaper spelling outside the N5 table) never fired.

## The engine facts this analysis rests on

Verified primary-source facts (re-checked while authoring this analysis, 2026-07-20):

| # | Fact | Evidence |
|---|---|---|
| F1 | templ has **no whitespace-control syntax** (no trim markers, no preserve flag) and its generator **collapses every run of whitespace in HTML text nodes to a single space** at `templ generate` time. | [a-h/templ#90](https://github.com/a-h/templ/issues/90) (closed as enhancement; no `--preserve-whitespace` shipped as of v0.3.1020); spike C §(d); templ.guide had no whitespace page (404 re-checked) |
...
| F4 | `<style>` element contents are rendered **without any changes** (CSS curly braces included), so a literal `<style>` block is authorable. *Note (E22 port landing):* the flip side is that a **component call cannot appear inside a `<style>` element** — its content is CSS raw text, not component territory — so a chrome fragment spliced inside a chrome-owned `<style>` (Heddle's `custom_styles`) is instead authored as a fragment **owning its `<style>` wrapper**, which is byte-neutral (the emitted `<style>/* CSS Comment Test */</style>` bytes are identical). | [templ.guide — CSS style management](https://templ.guide/syntax-and-usage/css-style-management/): "`<style>` element contents are rendered to the output without any changes." (doc claim — S1 probe P3 executes it; the wrapper-ownership note is the landed `composed_page.templ` component-boundary note) |
...
| F7 | *(added at the E20/E22 port landing, probed 2026-08-08 at v0.3.1020)* templ's generator **normalizes self-closing void elements to the slashless form** (`<img/>` / `<img />` → `<img>`) — a NON-whitespace divergence where the oracle spells ` />`. Accommodation: the affected sites (the oracle's nine composed-page ` />` sites and fragment-heavy's media `<img … />`) emit their exact pinned bytes via `@templ.Raw` over pinned template literals — the doctype-accommodation mechanism, never applied to model data ([go-port-mapping.md rule 8](go-port-mapping.md#controlled-track--templ)). The same probe confirmed: IE conditional comments pass through **byte-exact**; literal `&`, `™` and `'` in template text pass through unescaped; empty components and inline component calls work; dynamic `class=`/`href=` attribute expressions emit the expected bytes. Idiomatic templ authors void elements natively (slashless). | The landed controlled port (`internal/templeng/controlled/composed_page.templ` header + `fragment_heavy.templ` header, both generated at the pinned v0.3.1020 and byte-gate green); S1's P1 had already observed the slashless emission on `trivial-substitution`'s `<img>` — there the oracle was also slashless, so no accommodation was needed |
...
presence-vs-absence alike ([contract v2 — normalization pipeline](parity-contract-v2.md#normalization-pipeline);
Phase 6 D13).
...
The workload shapes were authored under exactly this discipline
([workloads.md — shared authoring rule 3](workloads.md#shared-authoring-rules-for-the-five-new-workloads):
...
and models in [workloads.md](workloads.md) and — for
`composed-page` — from the live twin sources
(`TwinContent.cs`,
[composed-page.fluid.liquid](../dotnet/templates/controlled/liquid/composed-page.fluid.liquid)
(the dialect-suffixed Liquid entry; the `.dotliquid.liquid` copy is byte-identical),
`AreaComponent.cs`,
all read for this analysis).
...
recorded result block in `benchmarks/go/README.md`. Prerequisite (held): Phase 1's corpus is
...
- **P1 — smallest-workload byte gate.** Author `trivial-substitution.templ` densely
  (per [go-port-mapping.md](go-port-mapping.md#workload-2--trivial-substitution)), `go tool templ generate`
  (the pinned CLI via `go tool`, per [D3](go-harness-and-measurement.md#layout-and-toolchain-rules-d2d3-condensed)),
...
- **P2 — escaper bytes.** Render a one-row fortunes-shaped fragment and an encoded-loop-shaped
  ...
  path; assert every spelling is in the [N5 table](parity-contract-v2.md#entity-canonicalization-n5)
...
**Completion check:** `benchmarks/go/README.md` carries an "S1 results" section recording, per
probe, the command, the observed bytes (hex excerpt where relevant), and the verdict.

### Expected outcome — pass (the analysis holds; this is the outcome that occurred)
...
nothing) that N3b reconciles. **Path (taken):** the templ controlled ports were
authored for all eight workloads, `composed-page` first (it was the historically-flagged cell),
each gated before timing — the landed ports are byte-gate green at the pinned toolchain (F7).
No further feasibility checkpoints existed — S1 was the only one.

### Contingency — a genuine non-whitespace divergence (did not fire; retained as the operative rule for re-runs)
...
contract's [exclusion policy](parity-contract-v2.md#exclusion-policy)
...
3. **Evidence file.** Write `benchmarks/go/EXCLUSIONS.md` with one section per excluded cell
   ...
   (honest-reporting rule 6, [metrics-protocol.md](metrics-protocol.md#honest-reporting-rules)).
...
5. **quicktemplate consequence.** Any templ controlled-track exclusion means templ is not
   "fully green", so the quicktemplate conditional stretch (D10) is automatically not
   triggered — no decision needed at that point; that consequence was decided in advance.
6. **Whitespace-only divergence never reaches this path (decision D13, CLOSED — option A).**
...
The expected outcome and the non-whitespace contingency were both fully specified in advance;
S1 confirmed and recorded evidence, it did not design.
