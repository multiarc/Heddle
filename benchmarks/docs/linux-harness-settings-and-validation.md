...
Harness-settings and validation documentation for the Linux cross-check (phase 8 of the
program). It pins the Linux stability settings for all six harnesses (each mirroring the
structure of its ecosystem's Windows-settings decision), the gate re-assertion procedure and
its failure triage, the measurement-run shapes, the two-part report format, the dimensionless
dispersion form, and the exact material-divergence thresholds. Decisions of record: D6–D17
(this document carries their normative text — `validate.py` cites D15, D16, and D17.4 here).

Everything below runs inside the one recorded machine state of
[linux-environment-and-toolchains.md — D2](linux-environment-and-toolchains.md#one-machine-state-for-every-linux-number-d2):
...
decision, and carries the same instability trigger its ecosystem's Windows decisions recorded
(re-targeted to Linux mechanisms). The statistic selection is the protocol's
[mapping](metrics-protocol.md#wall-time-statistic-mapping-q21)
...
- Temurin 25 Linux x64 per the toolchain table; the JMH `# VM version` line is the recorded
  ...
  matching Phase 3 (amendment E6; it was
...
- Launcher: `run-js.sh` in the phase's tooling (`benchmarks/linux-crosscheck/`; D18) replicates `run.ps1`'s capture and
...
### D10 — pyperf (Python; Windows source: Phase 5 D8 / the [python-harness.md run protocol](python-harness.md#run-protocol))
...
### D11 — Go testing/benchstat (Go; Windows source: the [Phase 6 stability-settings table](go-harness-and-measurement.md#repeated-runs-stability-settings-benchstat))
...
maintainer decision of 2026-07-20 recorded in Phase 1 D8
and [parity-contract-v2 §normalization-pipeline](parity-contract-v2.md#normalization-pipeline))**, N4,
...
6. **Upstream defect path:** if the diagnosis shows a defect in a shipped phase 1–7 artifact
   ...
   [amendments-ledger](../../docs/spec/common/cross-cutting-decisions.md#cross-spec-amendments-ledger)
...
**Tooling:** `validate.py` (`benchmarks/linux-crosscheck/validate.py`; D18) computes every table from (a) this run's artifacts and
...
Publication is blocked until all of the following pass (cited elsewhere as D17.1–D17.6 — the
program's success criteria made mechanical; the checklist result is recorded in the run log):
