...
Companion document of the JVM harness (`benchmarks/jvm/`). It pins every cell of the
workload × engine × track matrix: the Java model definitions (byte-compatible with the pinned
data of [Phase 1 workloads.md](workloads.md)), the construct
...
[parity contract](parity-contract-v2.md); the Thymeleaf
controlled authoring pattern is normative in the
[feasibility doc](jvm-thymeleaf-feasibility.md).
...
| 7 | fortunes-encoded | `${...}` under `FiveEntityHtmlOutput` (Html, D4) | `${...}` under stock OWASP output (Html; verifier per D6 amendment) | `[[...]]` escaped inlining — rung 2 | `th:text` on `<td>` elements |
...
| `LoopRow` | `value (int)` | 5,000 rows, `i` in `[0,4999]`: `i` — the row carries ONLY the ordinal; the display name `row-<i>` is composed by the templates as `row-` + the value substitution (E21) |
...
| `MixedProduct` | `name, skuNumber (int), price (int), onSale (boolean), batch (int)` | 36 rows, `i` in `[1,36]`: `String.format(Locale.ROOT, "Product %02d", i)`, `1000 + i`, `950 + i * 7`, `i % 3 == 0`, `i` — the templates compose the display SKU (`MX-` + `skuNumber`) and the blurb sentence around `batch` (E21) |
| `ConditionalRow` | `name, seq (int), bronze, silver, gold, hasNote, active (booleans)` | 200 rows, `i` in `[0,199]`: `String.format(Locale.ROOT, "unit-%03d", i)`, `i`, `i % 4 == 0`, `i % 4 == 1`, `i % 4 == 2`, `i % 2 == 0`, `i % 5 != 0` — the templates compose the note text (`note ` + `seq`) (E21); the zero-padded identity name stays model-side by design |
| `FragmentRow` | `kind, tile/card/media/stat (booleans, getters `isTile()` …), name, value (int), badge, delta (int), promo (FragmentPromo)` | 48 rows, `i` in `[0,47]` (E20 redesign): `kind = {"tile","card","media","stat"}[i % 4]` (informational — engines dispatch on the precomputed booleans, never the string), `String.format(Locale.ROOT, "item-%02d", i)`, `i * 11`, `{"new","hot","sale","std"}[i % 4]`, `i % 7 - 3`, promo on EVERY row (no null guard). Derived display text (media caption `Caption for ` + name, image src `/img/` + name + `.jpg`, display price + `.99`) is composed by the TEMPLATES (E21) |
| `FragmentPromo` | `label, price (int)` | `label == badge`, `price = 9 + i` — whole-currency int; the templates compose the display price |
| `FortuneRow` | `id (int), message` | the 12 pinned rows of [workloads.md workload 7](workloads.md#workload-7--fortunes-encoded-encoded), transcribed byte-for-byte (row 1 `4.33e67` — no `+`; rows 4/8 em dash U+2014; row 11 the exact XSS payload; row 12 `フレームワークのベンチマーク`); source files saved UTF-8 without BOM |
| `EncodedLoopRow` | `tag, name, comment` | 5,000 rows, `i` in `[0,4999]`: `"tag-" + i + "&'" + (i % 7) + "'"`, `"item <" + i + "> & \"co\""`, `"'q' & <angle> \"d\" こんにちは " + i` |
| `ComposedModel` | `nav (NavModel)` — and NOTHING else (E20 structured nav; E22 removed the text half) | loaded once at static init (lazy holder) from `GoldenCorpus/fixtures/composed-page/nav.json` via the gate's corpus-dir resolution (`Corpus.resolveRoot()`, honoring `-Dheddle.corpus`) |
...
[workloads.md workload 4](workloads.md#workload-4--mixed-page-raw)
...
**D4 — the custom `FiveEntityHtmlOutput`** (folded from the phase README; cited by
`benchmarks/jvm/src/main/java/heddle/benchmarks/jvm/jte/FiveEntityHtmlOutput.java`). The two
controlled encoded cells render with the `ContentType.Html` engine into
`heddle.benchmarks.jvm.jte.FiveEntityHtmlOutput`, a class implementing
`gg.jte.html.HtmlTemplateOutput` over an internal `StringBuilder`: `writeUserContent(String)`
escapes exactly `&`→`&amp;`, `<`→`&lt;`, `>`→`&gt;`, `"`→`&quot;`, `'`→`&#39;` in **both**
tag-body and attribute contexts; all `writeContent` overloads append verbatim; primitive
`writeUserContent` overloads append their `String.valueOf` (never escapable); it records
`setContext(tagName, attributeName)` and **throws** `IllegalStateException` if a `script`-tag
or `on*`-attribute JavaScript context is ever entered — a tripwire, not a path, since no such
context exists in the corpus templates. Why: stock `OwaspHtmlTemplateOutput` escapes only
`& < >` in tag bodies — `'` and `"` pass through raw in text context, a genuine
beyond-whitespace, beyond-spelling divergence from the oracle that N5 cannot reconcile (N5
canonicalizes *spellings of escaped* characters, never characters an engine chose not to
escape). `TemplateEngine.checkOutput` uses a supplied `HtmlTemplateOutput` as-is (verified at
source), so this is a supported engine seam, not a fork or a post-processing step — the same
pattern as Phase 1's Handlebars.Net custom `ITextEncoder` — and emitting the canonical
spellings directly renders N5 an identity transform on controlled JTE output. The controlled
track's disclosed non-idiomatic authoring license covers it; the report discloses it in the
JTE row notes.
...
[Phase 1 golden-corpus.md](golden-corpus.md#idiomatic-verifier-definitions)
(rationale in D6).
...
`TemplateMode.HTML` — D2).
Controlled templates follow the
[block-only pattern](jvm-thymeleaf-feasibility.md#the-mechanism-set-all-core-standard-dialect-all-cited)
...
- Every controlled template's dynamic sites correspond one-to-one with the Heddle normative
  ...
  [workloads.md](workloads.md) — same substitution count,
