...
Companion document of the JVM harness. It pins the `benchmarks/jvm/`
...
assembled. Decisions of record (origin tags from the closed program): D2, D3, D8–D12.
...
benchmarks/jvm/
  ...
  README.md                                ← quick-start; links to this document
...
[idiomatic-track gate](parity-contract-v2.md#idiomatic-track-gate):
...
[golden-corpus.md — verification](golden-corpus.md#verification),
...
3. Full run (single invocation, all benchmarks, GC profiler, JSON + log capture):
   ...
   The committed annotations (`@Fork(5)`, 1 × 2 s warmup, 5 × 1 s measurement) encode the
   `short` budget as resized by amendment E14 (~10 min per engine; an engine is 16 cells —
   eight workloads × two tracks; E14 spends the JVM increment on forks, its dominant
   variance term). The `baseline` budget layers `-f 5 -wi 2 -i 17` on the same jar for
   ~30 min per engine (what `run-all.sh --budget baseline` passes). Still run it on the
   quiet box — a shorter regime reduces the *reservation*, not the quiet-machine
   requirement.
