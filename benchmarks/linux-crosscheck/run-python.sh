#!/usr/bin/env bash
# run-python.sh — pyperf launcher.
# Runner settings unchanged from Windows (pyperf defaults:
...
# the topology at run time, never assumed — the affinity-target delta vs Windows
# --affinity=4 is disclosed in the environment notes). No elevated shell, no priority:
# the Linux counterpart of REALTIME_PRIORITY_CLASS is the tune + isolation itself.
# Memory pass (mem_tracemalloc.py) runs identically, separate from timing.
#
...
lcx_mkout
lcx_launcher_header "python (pyperf)" 2>&1 | tee "$LCX_OUT_DIR/run-python.log"
...
# Gates first: selftest + both gate tracks, the ecosystem's own commands.
...
# Affinity: the isolated SMT pair, read (never assumed) from the topology.
AFFINITY_ARGS=()
if PAIR="$(lcx_smt_pair)" && [ -n "$(cat /sys/devices/system/cpu/isolated 2>/dev/null)" ]; then
  ...
  if [ "$LCX_NO_TUNE_CHECK" = "1" ]; then
    lcx_note "RECORDED: no isolated pair available (WSL) — --affinity omitted; functional run only" 2>&1 | tee -a "$LCX_OUT_DIR/run-python.log"
  else
...
# The five bench scripts, same JSON outputs + pyperf stats dumps as the Windows runs.
...
# Memory pass — separate, never combined with timing.
