#!/usr/bin/env bash
# tune.sh — enter the one measurement-session state (isolation precondition,
# machine-wide pyperf tune, boost off, state capture) and assert every
# post-condition (governor, boost=0, sample rate, isolated set).
#
# BARE-METAL ONLY. Under WSL2 this script fails cleanly by design — WSL
...
fail_bare_metal_only() {
  ...
  echo "tune.sh: the tuned session state (pyperf system tune, amd-pstate boost-off, isolcpus pair)" >&2
  echo "tune.sh: exists only on the bare-metal Ubuntu Server 24.04 boot of the protocol box." >&2
  echo "tune.sh: Under WSL2 this failure is the designed outcome; the tune/untune work" >&2
  echo "tune.sh: is deferred to bare metal, not faked here." >&2
  exit 1
}

# --- Step 0: refuse WSL up front (designed failure) ----------------------------
if lcx_is_wsl; then
  fail_bare_metal_only "WSL2 detected via /proc/version"
fi

# --- Step 1 precondition: isolation boot entry active -------------------------
# The SMT pair is READ from the topology, never assumed.
...
if [ -z "$ISOLATED" ]; then
  ...
  echo "tune.sh: precondition; see grub-isolation.cfg.example)." >&2
  exit 1
fi
...
lcx_note "running: python3 -m pyperf system tune (machine-wide)"
...
# --- Post-condition asserts (fail if any item does not read back) --------------
assert_eq() { # label expected actual
  if [ "$2" != "$3" ]; then
    ...
    echo "tune.sh: bare-metal measurement boot (bare-metal-only operation, deferred under WSL)." >&2
    exit 1
  fi
...
# perf sample rate: pyperf system tune sets 1.
...
lcx_note "capturing 'pyperf system show' -> $LCX_STATE_FILE (authoritative session-state record)"
python3 -m pyperf system show | tee "$LCX_STATE_FILE"

lcx_note "Session state entered. Isolated pair: ${PAIR}. Run capture-environment.sh next."
