...
```bash
# build (warning-clean)
dotnet build benchmarks/third-party/Heddle.Benchmarks.ThirdParty.csproj -c Release
...
dotnet run -c Release --project benchmarks/third-party -- dump

# the full head-to-head (BenchmarkDotNet; writes to ./BenchmarkDotNet.Artifacts under this folder)
dotnet run -c Release --project benchmarks/third-party -- --filter *HeddleComparisonBenchmarks*
```
