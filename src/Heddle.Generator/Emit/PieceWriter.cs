...
{
    /// <summary>The single static-piece emission hook. Every static piece is emitted as the string
    ...
    /// one hook, the same generated type serves the v1 string path and the byte sink.
    /// <para>This file states <i>what</i> a piece emits, never <i>how</i> a literal is escaped or how a surrogate is
    /// found — both are <see cref="CSharpEscape"/>'s duties. The local <c>Escape</c>/<c>HasLoneSurrogate</c>/<c>IndexOfLoneSurrogate</c>
    /// members were the last generator-side spellings of that knowledge and are gone.</para></summary>
