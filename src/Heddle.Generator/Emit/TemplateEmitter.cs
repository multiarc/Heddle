...
namespace Heddle.Generator.Emit
{
    ...
    internal sealed class TemplateEmitter
    {
        ...
        /// absolute in a real build). The generated file states which, because a reader of a <c>#line</c> otherwise
        /// cannot tell, and the two forms resolve against different working directories.</summary>
        ...
        public TemplateEmitter(string key, string sanitizedName, string generatedNamespace, string cleanDocument,
            ...
            bool lineDirectiveFileIsRootRelative = true)
        {
            _key = key;
        ...
        private string RenderFile(string modelType, BodyClass root)
        {
            ...
            // Q8.27: mark the relativity of the #line file names, so a reader never has to guess which form they are
            // in. Outside the template root the emitted name is the template's own (absolute, in a real build) path;
            // under the root it is root-relative, because an absolute path there would bake one machine's layout into
            // every checked-in generated-source golden.
            w.Raw(_lineDirectiveFileIsRootRelative
                ? "// #line file names below are RELATIVE to HeddleTemplateRoot."
                : "// #line file names below are the template's own path (no HeddleTemplateRoot anchor applies).");
        ...
        private string BuildManifestEntry(string contentHash, bool isDynamic, string modelType, bool marker)
        {
            ...
            sb.Append(marker ? "    strategy: null)" : $"    strategy: {entryType}.Root)");
            return sb.ToString();
        }
        ...
        private string ExtensionBindingsArray()
        {
            ...
            sb.Append(string.Join(", ", _extensionBindings
                ...
                .Select(b => "new global::Heddle.Precompiled.PrecompiledExtensionBinding(" +
                             ...
                             // Phase 3 (OQ4): the additive prop-layout row (schema 4). Omitted entirely for a
