...
namespace Heddle.LanguageServices
{
    ...
    /// <para>A wrapper over the shared <see cref="LineIndex"/> (generator plan phase 6 D6), which owns the rule
    /// this comment used to restate: <c>\n</c> terminates a line and a <c>\r</c> adjacent to it belongs to the
    /// terminated line. The engine's <see cref="HeddleCompileResult"/> now indexes through the same type, so the
    /// equivalence claim holds by construction rather than by convention.</para>
    /// </summary>
