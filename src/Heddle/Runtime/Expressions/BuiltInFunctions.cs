...
namespace Heddle.Runtime.Expressions
{
    ...
    internal static class BuiltInFunctions
    {
        ...
        internal static ForModel Range(int start, int last) => new ForModel { Start = start, Last = last };
        ...
        /// </summary>
        internal static ForModel Range(int start, int last, int step)
        {
            if (step <= 0)
                throw new TemplateProcessingException(string.Format(CultureInfo.InvariantCulture,
                    RangeStepMessageFormat, step));
            return new ForModel { Start = start, Last = last, Step = step };
        }
