...
namespace Heddle.Language.Expressions
{
    ...
    /// <summary>
    /// Heddle's function-overload selection rule, stated once (phase 4 D10 / 04 F3): rank exact 0, widening /
    /// reference / lifting 1 — <b>flat</b> — and boxing to <c>object</c> 2, with params-expanded arguments ranked
    /// one worse; then take the Pareto non-dominated front and call any non-singleton front ambiguous.
    ...
    /// rank "by construction" — and that claim was false, because the rank vector here is flat while C# betterness
    /// prefers the closest target. <c>min(1, 2u)</c> is the shipped counter-example: <c>(long,long)</c>,
    /// <c>(double,double)</c> and <c>(decimal,decimal)</c> all rank <c>(1,1)</c>, so the runtime raises HED1013
    /// while C# happily picks <c>Min(long,long)</c> and renders.</para>
    /// <para>OQ2 (resolved user, 2026-07-25): the flat Pareto rank stays the semantics of record. The generator
    /// therefore consults this core as an emit guard — degrade on ambiguity, and emit cast-pinned to the chosen
    /// signature otherwise, which makes the "by construction" claim actually true.</para>
    /// </summary>
