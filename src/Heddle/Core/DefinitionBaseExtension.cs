...
namespace Heddle.Core
{
    internal class DefinitionBaseExtension : AbstractExtension
    {
        ...
        // Precompiled props carriage; required because BindDefinition bypasses InitStart.
        private object[] _precompiledPropsPrototype;
        private PrecompiledPropSetter[] _precompiledPropSetters;

        /// <summary>The per-call-site props binder, or <c>null</c> for a prop-less definition (the fast
        ...
        /// <summary>Assigns the recursion limit that <see cref="InitStart"/> would otherwise set from
        ...
        /// <summary>Installs the precompiled props carriage (frozen prototype + dynamic setters) that the
        /// runtime backend would produce via <c>PropsBinder</c>. The frozen prototype is shared when all-constant;
        /// otherwise it is cloned per invocation and each setter runs against the caller view.</summary>
        ...
        /// <summary>True when the invoked definition declares a slot parameter. In slot mode the caller
        ...
        /// The slot-projection entry point for <c>OutExtension</c>.</summary>
