...
namespace Heddle.TestCorpus
{
    ...
    internal static class CorpusIntent
    {
        ...
        public static readonly IReadOnlyList<CorpusIntentRow> Rows = new[]
        {
            ...
            new CorpusIntentRow("ext-site-fallback-twice.heddle", CorpusTier.CompilesWithRefusal,
                CorpusRender.Standalone,
                "Two per-site refusals in one document: two @scanner calls, so the record carries two UnsupportedExtension sites whose Sites rows have distinct walk ordinals and whose payloads index the row's own RefusalSites list — the shape that once broke the writer's payload bound.",
                refusals: RefusalClass.UnsupportedExtension),
            new CorpusIntentRow("refusal-fragment-body.heddle", CorpusTier.CompilesWithRefusal,
                CorpusRender.WithModel,
                "A refused site with a BODY, so the loader's recompile of the recorded source is not one call but a nested compile of its own: the fragment's @list body reads the element and roots a '::' read at the document, and both have to resolve under the context the fragment inherits from the site it replaced. The sweep byte-compares all of it against the dynamic tier.",
                refusals: RefusalClass.UnsupportedExtension),
            new CorpusIntentRow("refusal-nested-body.heddle", CorpusTier.CompilesWithRefusal,
                CorpusRender.WithModel,
                "The twin of the row above with the nesting the other way round: the refused site sits INSIDE a @list element body, so its recorded source is sliced out of a document whose own text starts partway into the template. A site at a document's top level reads the same either way, which is why every other refusal row here missed the coordinates being wrong; this one renders the element's own member and roots a '::' read at the document from inside the fragment, and the sweep byte-compares both against the dynamic tier.",
                refusals: RefusalClass.UnsupportedExtension),
            new CorpusIntentRow("refusal-import-lib.heddle", CorpusTier.CompilesWithRefusal,
                CorpusRender.WithModel,
                "The library half of the import-refusal pair, and a refusal row in its own right: a refused site at a document's top level and another inside a @list element body. Composed into the page row below, both of them compile into the PAGE's documents while their positions stay absolute in this file — the coordinates neither of the page's recorded texts carries, which is the whole point of the pair.",
                refusals: RefusalClass.UnsupportedExtension),
            new CorpusIntentRow("refusal-import-page.heddle", CorpusTier.CompilesWithRefusal,
                CorpusRender.WithModel,
                "A refused call reached through an '@<<' composition import. The import expands inline, so the refused sites get no document of their own and are positioned in a file this row does not contain; the record slices their source out of the text the parse consumed and reports each site at the import block, which is the only position in THIS template a reader can act on. The sweep byte-compares both tiers, which is what says the recompiled fragments still stand where the imported calls stood.",
                refusals: RefusalClass.UnsupportedExtension),
            new CorpusIntentRow("def-default-body-accessor.heddle", CorpusTier.Compiles, CorpusRender.WithModel,
                "A definition whose default body reads a root-model member, called with no content: the default body is served at load as an alternate body under the definition's template, so its accessor must have a Sites row from the walk or strict load would silently compile it."),
