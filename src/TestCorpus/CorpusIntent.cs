    ...
    internal static class CorpusIntent
    {
        ...
        public static readonly IReadOnlyList<CorpusIntentRow> Rows = new[]
        {
            ...
            new CorpusIntentRow("ext-bodied-custom.heddle", CorpusTier.FallsBackSafely, CorpusRender.Standalone,
                "A bodied call to a REFERENCED third-party extension whose InitStart has the step-back shape — the case that shows what the build tier still cannot do. The build has not read that hook, cannot reproduce it, and degrades under the HED7015 warning. There is no build property that changes this: an earlier attempt to answer it by loading the extension and running its hook out of band has been deleted, and the answer is to run the extension's own InitStart in the consumer's assembly at type-init instead. Nothing about the language blocks it — only what this build can see."),
            new CorpusIntentRow("ext-bodied-unemittable.heddle", CorpusTier.FallsBackSafely, CorpusRender.Standalone,
                "The boundary beside it: a bodied call to a referenced extension whose hook is perfectly readable and whose answer is a body typed by the call VALUE, a role the emitter has no emission for. It degrades however the hook is read, which is the point — reading a hook is not a licence to emit one, and an unemittable role costs the template its tier rather than being guessed into something emittable."),
