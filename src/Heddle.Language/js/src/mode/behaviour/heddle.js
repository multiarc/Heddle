...
var TTLstyleBehaviour = function() {
    this.add("braces", "insertion", function(state, action, editor, session, text) {
        ...
        if (text == '{{') {
            ...
            initContext(editor);
            var rightChar = line.substring(cursor.column, cursor.column + 2);
            if (rightChar == '}}') {
                var matching = session.$findOpeningBracket('}}', {column: cursor.column + 2, row: cursor.row});
            ...
            var rightChar = line.substring(cursor.column, cursor.column + 2);
            if (rightChar === '}}') {
                var openBracePos = session.findMatchingBracket({row: cursor.row, column: cursor.column+2}, '}}');
                if (!openBracePos)
                     return null;
    ...
    this.add("braces", "deletion", function(state, action, editor, session, range) {
        ...
        if (!range.isMultiLine() && selected == '{{') {
            ...
            var rightChar = line.substring(range.end.column, range.end.column + 2);
            if (rightChar == '}}') {
                range.end.column += 1;
                return range;
            } else {
    ...
    this.add("brackets", "deletion", function(state, action, editor, session, range) {
        ...
        if (!range.isMultiLine() && selected == '@%') {
            initContext(editor);
            var line = session.doc.getLine(range.start.row);
            var rightChar = line.substring(range.start.column + 2, range.start.column + 4);
            if (rightChar == '%@') {
                range.end.column += 2;
                return range;
            }
