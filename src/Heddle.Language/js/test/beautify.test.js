...
test("golden: @@ escapes are preserved verbatim and idempotent", () => {
    const src = "a @@ b @@@@ c @@@(Name)";
    const { once, twice } = beautifyTwice(src);
    assert.strictEqual(once, src, "escape content must not be rewritten");
    assert.strictEqual(twice, once, "beautify must be idempotent on @@ content");
});

test("golden: region definitions are preserved verbatim and idempotent", () => {
    const src = "@%<:header>{{Default}}%@";
    const { once, twice } = beautifyTwice(src);
    assert.strictEqual(once, src);
    assert.strictEqual(twice, once);
});

test("golden: typed region header spacing is preserved and idempotent", () => {
    const src = "@%<:item :: Article>{{Body}}%@";
    const { once, twice } = beautifyTwice(src);
    assert.strictEqual(once, src);
    assert.strictEqual(twice, once);
});

test("golden: region fill inside a call body is idempotent", () => {
    const src = "@card(A){{ @%<header:header>{{H}}%@ }}";
    const { once, twice } = beautifyTwice(src);
    assert.strictEqual(twice, once, "beautify must be idempotent on region fills");
});
