...
test("@@ escape -> constant.character.escape (tmLanguage escaped-at parity)", () => {
    const tok = tokenFor("a @@ b", "@@");
    assert.strictEqual(tok.type, "heddle-sub.constant.character.escape");
});

test("@@ even run: each pair is its own escape token", () => {
    const escapes = classified("@@@@").filter((t) => t.aceClass === "constant.character.escape");
    assert.strictEqual(escapes.length, 2);
    assert.deepStrictEqual(escapes.map((t) => t.value), ["@@", "@@"]);
});

test("@@ odd run: the pair escapes, the trailing @ starts a directive", () => {
    const toks = classified("@@@(x)");
    assert.strictEqual(toks[0].value, "@@");
    assert.strictEqual(toks[0].aceClass, "constant.character.escape");
    assert.strictEqual(toks[1].value, "@");
    assert.strictEqual(toks[1].aceClass, "support.function");
});

test("@@* comment adjacency: the escape must NOT fire (grammar predicate parity)", () => {
    // `@@*c*@` is `@` (directive start) followed by the comment `@*c*@` — the
    // second `@` begins the comment, so no escape token may be produced.
    const toks = classified("@@*c*@x");
    assert.ok(
        !toks.some((t) => t.aceClass === "constant.character.escape"),
        "no escape token expected before a comment: " + JSON.stringify(toks)
    );
    assert.strictEqual(toks[0].value, "@");
    assert.strictEqual(toks[0].aceClass, "support.function");
    const commentStart = toks.find((t) => t.value === "@*");
    assert.ok(commentStart, "expected a comment-start token");
    assert.match(commentStart.aceClass, /comment/);
});

test("@@ escape inside a {{ }} body (SUB_AT_ESCAPE mirror)", () => {
    const tok = tokenFor("@if(x) {{ a @@ b }}", "@@");
    assert.strictEqual(tok.aceClass, "constant.character.escape");
});
...
test("region definition <:name>: visibility ':' + name classes", () => {
    const toks = classified("@%<:header>{{Default}}%@");
    const colon = toks.find((t) => t.value === ":");
    assert.ok(colon, "expected the visibility ':' token");
    assert.strictEqual(colon.type, "heddle-def-name.keyword.operator");
    const name = toks.find((t) => t.value === "header");
    assert.strictEqual(name.type, "heddle-def-name.identifier");
});

test("typed region <:item :: Article>: '::' operator + storage.type", () => {
    const toks = classified("@%<:item :: Article>{{Body}}%@");
    const dblcolon = toks.find((t) => t.value === "::");
    assert.ok(dblcolon, "expected a single '::' token");
    assert.strictEqual(dblcolon.type, "heddle-def-name.keyword.operator");
    const type = toks.find((t) => t.value === "Article");
    assert.strictEqual(type.aceClass, "storage.type");
    const name = toks.find((t) => t.value === "item");
    assert.strictEqual(name.type, "heddle-def-name.identifier");
});

test("generic-typed region <:item :: List<Article>> routes through generic-type", () => {
    const toks = classified("@%<:item :: List<Article>>{{B}}%@");
    const open = toks.find((t) => t.value === "List<");
    assert.ok(open, "expected the generic open token");
    assert.strictEqual(open.aceClass, "storage.type");
    const arg = toks.find((t) => t.value === "Article");
    assert.strictEqual(arg.aceClass, "storage.type");
});

test("region fill <name:name> keeps the override classes", () => {
    const toks = classified("@%<header:header>{{override}}%@");
    const names = toks.filter((t) => t.value === "header");
    assert.strictEqual(names.length, 2);
    for (const n of names) {
        assert.strictEqual(n.type, "heddle-def-name.identifier");
    }
    const colon = toks.find((t) => t.value === ":");
    assert.strictEqual(colon.type, "heddle-def-name.keyword.operator");
});

test("region tokens after `>` return to the definition state ({{ body folds out)", () => {
    const toks = classified("@%<:item :: Article>{{Body}}%@");
    const close = toks.find((t) => t.value === ">");
    assert.strictEqual(close.type, "heddle-def-name.punctuation.operator.paren.rparen");
    const bodyOpen = toks.find((t) => t.value === "{{");
    assert.strictEqual(bodyOpen.type, "heddle-def.keyword.operator.paren.lparen");
});
