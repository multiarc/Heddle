...
- Gate (one combined run): `dotnet build -c Release` → `dotnet test src/Heddle.Tests` (all TFMs) → no diff under `src/Heddle.Language/generated/` (unless the spec licenses a grammar change) → benchmarks when a hot path is touched → `npm run docs:build` when docs pages changed.
